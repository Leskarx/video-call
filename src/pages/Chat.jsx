...
import IncomingCall from "../components/IncommingCall.jsx";
...
export default function Chat() {
  ...
  const handleIncommingCall = useCallback(({ offer, id }) => {
    ...
    setIncomingCall({ offer, callerId: id }); // 🔥 Save incoming call details
  }, []);
  ...
  useEffect(() => {
    ...
    socket.on("call-accepted", handleCallAccepcted);
    socket.emit("get-users");

    return () => {
      socket.off("joined-users", handleJoinedUsers);
      socket.off("incomming-call", handleIncommingCall);
      socket.off("call-accepted", handleCallAccepcted);
    };
  }, [socket, handleJoinedUsers, navigate, handleIncommingCall,handleCallAccepcted]);
  ...
  const endCall = () => {
    ...
    setIncomingCall(null); // Reset incoming call UI
  ...
  const answerCall = async () => {
    ...
    socket.emit("answer-call", { answer, to: incomingCall.callerId }); //sent the answer to the caller
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      {/* Incoming Call UI */}
      {incomingCall && (
        <IncomingCall answerCall={answerCall} endCall={endCall} />
      )}
    </div>
  );
}
